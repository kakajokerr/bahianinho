...
module.exports = msgHandler = async (client, message) => {
    try {
        ...
        const mess = {
            wait: '[ 1 MIN. ] aguarde até ser feito.',
            error: {
                St: '[❗] para fazer sua figura digite *!sticker*, para sticker animado, menos de 5 seg.',
                Qm: '[❗] Ocorreu um erro, talvez o tema não esteja disponível!', ',
                Yt3: '[❗] Algo deu errado, não é possível converter para mp3!',
                Yt4: '[❗] Ocorreu um erro, talvez o erro seja causado pelo sistema.',
                Ig: '[❗] Ocorreu um erro, talvez porque a conta é privada',
                Ki: '[❗] O bot não pode remover o adm desse grupo!',
                Ad: '[❗] O bot Precisa de adm para este comando.',
        ...
        case '!sgif':
            if (isMedia) {
                if (mimetype === 'video/mp4' && message.duration < 10 || mimetype === 'image/gif' && message.duration < 10) {
                    const mediaData = await decryptMedia(message, uaOverride)
                    client.reply(from, 'Em andamento⏳ aguarde ± 1 min!', id)
                    ...
                    client.reply(from, '[❗] Envie um vídeo com a legenda *! StickerGif * max 5 sec! ', id)
                )
            }
            break
	    case '!stickernobg':
